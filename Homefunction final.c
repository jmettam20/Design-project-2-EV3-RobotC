...
void home()
{
	...
	while(edge != 5)//jump in to loop
	{
		...
		setMotorSync(leftMotor, rightMotor, 0, 100);//move forward
	}
	//jump out of loop
	resetMotorEncoder(leftMotor);//reset measurement
	setMotorSync(leftMotor,rightMotor, 0 , -100);//move backwards
	...
	setMotorSyncEncoder(leftMotor,rightMotor,0,(distanceTot/2), 100);//move forward half total
	...
	setMotorSyncEncoder(leftMotor,rightMotor,0,(distanceWidth/2), 100);//move forward half total width
	waitUntilMotorStop(leftMotor);//wait
	turnTarget(1,90);
}
